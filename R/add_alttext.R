...
#' @param alttext_csv Path to file containing labels, alt text, and captions
#'
...
#' text needs to be found in either 1)_the rda files produced from
...
#' 
#' Debugging tip: If you get an error "Paragraph ended before HyPsd@@ProtectSpacesFi was complete" along with a line number, the figure near that line does not have alt text in the csv file. 
...
add_alttext <- function(
  ...
  for (i in 1:nrow(alttext)) {
    ...
    label_line_idx <- grep(paste0("\\\\caption\\{\\\\label\\{", label, "\\}"), tex_file)
    if(length(label_line_idx) == 0) {
      stop(paste('The following figure label does not appear in the document:', label))
    } 
    ...
    tex_file[fig_line_idx] <- paste(line, "{", alttext$alt_text[i], "}", sep = "")
  }
  ...
  if (compile) {
    ...
    withr::with_dir( # changes the working directory only for rendering the tex file
      dir,
      tinytex::lualatex(file.path(ifelse(!is.null(rename), glue::glue("{rename}.tex"), x)))
    )
